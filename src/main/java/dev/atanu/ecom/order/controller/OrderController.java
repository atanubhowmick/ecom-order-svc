...
public class OrderController {
	...
	@ApiOperation(value = "Get all Order Details by User Id", response = GenericResponse.class)
	@PostMapping(value = "/create/{user-id}", produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<GenericResponse<OrderDetails>> createOrder(
			@ApiParam(value = "User Id", required = true) @PathVariable("user-id") Long userId,
			@RequestBody @Valid OrderDetails orderDetails) {
		GenericResponse<OrderDetails> response = new GenericResponse<>(orderService.create(userId, orderDetails));
		return new ResponseEntity<>(response, HttpStatus.OK);
	}
}
